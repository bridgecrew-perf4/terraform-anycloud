...
$fileContent = Get-Content .\config.yml
$content = ''
...
if (-not (Test-Path .\projects\$($yaml.defaults.project)\$($yaml.defaults.environment))) {
    New-Item -ItemType Directory .\projects\$($yaml.defaults.project)\$($yaml.defaults.environment)
}

foreach ($item in $(Get-ChildItem -Path .\templates)) {
    Copy-Item -Path .\templates\$item -Destination .\projects\$($yaml.defaults.project)\$($yaml.defaults.environment)\
}
Copy-Item -Path .\templates\variables.tf -Destination .\projects\$($yaml.defaults.project)\$($yaml.defaults.environment)\variables.tf

foreach ($item in $vars.GetEnumerator()) {
    Find-InTextFile -FilePath .\projects\$($yaml.defaults.project)\$($yaml.defaults.environment)\variables.tf -Find $item.Key -Replace $item.Value
}